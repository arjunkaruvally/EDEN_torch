...
int get_capacity(int n_neurons, int n_sequences, int max_memories, float alpha_s, float alpha_c, float beta, float T_f, float T_d, float c,
                 float epsilon, uint64_t seed, uint64_t time_index, float tolerance, int capacity_type){
    int u = max_memories;
    int l = 1;
    int cur_memories = 0;
    float success_ratio = 0;
    int n_seeds = 10;
    progressbar bar(n_seeds);
    while(u > l){
        cur_memories = (u+l)/2;
        success_ratio = 0;

        std::cout<<"testing for "<<cur_memories<<" memories : ";
        bar.reset();
        for(int seed=0; seed<n_seeds; seed++){
            EDEN obj = EDEN(n_neurons, cur_memories, n_sequences, alpha_s, alpha_c, beta, T_f, T_d, seed);
            obj.VERBOSE = false;
            float single_step = 0;

            switch (capacity_type) {
                case 1:
                    single_step = obj.fraction_fixed_point_success(epsilon);
                    break;
                case 2:
                    single_step = obj.fraction_single_step_success(epsilon);
                    break;
                default:
                    std::cout<<"Unknown capacity type";
            }

            success_ratio += single_step;

            if(obj.VERBOSE){
                diagnostic(obj, time_index);
            }

            bar.update();
        }

        success_ratio /= (1.0*n_seeds);

        std::cout<<" success ratio: "<<success_ratio<<std::endl;

        if(success_ratio >= 1-c){
            l = cur_memories+1;
            if(1.0*(u - l)/l < tolerance){
                break;
            }
        }
        else{
...
int main(int argc, char* argv[]) {
    ...
    options.add_options()
            ...
            ("max_memories", "Maximum number of memories to store", cxxopts::value<int>()->default_value("100"))
    ...
    int max_memories = result["max_memories"].as<int>();
    ...
    std::cout << "Neurons: " << n_neurons << ", Maximum memories: "<< max_memories << ", Sequences: " << n_sequences << std::endl;
    std::cout << "Alpha_s: " << alpha_s << ", Alpha_c: " << alpha_c << ", beta: "<< beta;
    std::cout << ", T_f: " << T_f << ", T_d: " << T_d << ", seed: " << seed << std::endl;

    int capacity = get_capacity(n_neurons, n_sequences, max_memories, alpha_s, alpha_c, beta, T_f, T_d, c,
                                epsilon, seed, result["time_index"].as<uint64_t>(), tol, capacity_type);

    std::cout<<"Capacity of the Network: "<<capacity<<std::endl;
    ...
    if (result_file.is_open()){
        ...
        result_file << "\"n_memories\": "<<capacity<<"\n";
        result_file << "} \n";
        result_file.close();
    } else {
